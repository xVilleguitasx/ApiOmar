...
class ProgramDetalleController {
    ...
    public async getOne(req: Request, res: Response): Promise<any> {
        const { id } = req.params;
        const games = await pool.query('SELECT * FROM programa_detalle WHERE id = ?', [id]);
    ...
    public async create(req: Request, res: Response): Promise<void> {
        ...
        const result = await pool.query('INSERT INTO programa_detalle set ?', [req.body]);
    ...
    public async update(req: Request, res: Response): Promise<void> {
        ...
        await pool.query('UPDATE programa_detalle set ? WHERE id = ?', [req.body, id]);
